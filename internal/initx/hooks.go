...
// settings JSON at path. The merge is additive and idempotent: existing
// hooks (Shale's or anyone else's) are preserved; ours are added once.
func InstallClaudeHooks(path string) (changed bool, err error) {
	...
	const command = "shale capture claude-code"
	for _, he := range claudeHookEvents {
		entries, _ := hooks[he.Event].([]any)
		if hasShaleHook(entries, command) {
			continue
		}
